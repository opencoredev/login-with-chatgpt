...
| `400` | `{ "error": "invalid_realtime_request", "message": "…" }` | `/realtime` |
| `400` | `{ "error": "invalid_service_tier", "serviceTier": "…" }` or `{ "error": "invalid_reasoning_effort", "reasoningEffort": "…" }` | `/responses` |
| `401` | `{ "error": "not_authenticated" }` | `/models`, `/responses`, `/realtime` |
...
| `413` | `{ "error": "realtime_request_too_large", "maxRequestBytes": … }` | `/realtime` |
